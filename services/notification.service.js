...
notificationService.updateUserToken = async (userId, fcmToken) => {
  try {
      ...
      try {
        ...
        if (user.assignedState) {
          await admin
            ...
            .subscribeToTopic(
              fcmToken,
              `state_${user.assignedState.toLowerCase().replace(/\s+/g, "_")}`,
            );
        }
        ...
        if (user.assignedDistrict) {
          await admin
            ...
            .subscribeToTopic(
              fcmToken,
              `district_${user.assignedDistrict.toLowerCase().replace(/\s+/g, "_")}`,
            );
        }
    ...
    return {
      success: true,
      message: "FCM token updated (Firebase not initialized)",
    };
  } catch (error) {
