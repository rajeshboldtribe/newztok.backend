...
userController.getMyProfile = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.error(httpStatus.NOT_FOUND, false, "User profile not found");
    }
...
userController.getEditorProfile = async (req, res) => {
  try {
    ...
    if (!editor) {
      return res.error(httpStatus.NOT_FOUND, false, "Editor profile not found");
    }
...
userController.getAdminProfile = async (req, res) => {
  try {
    ...
    if (!admin) {
      return res.error(httpStatus.NOT_FOUND, false, "Admin profile not found");
    }
...
userController.updateFcmToken = async (req, res) => {
  try {
    ...
    if (!fcmToken) {
      return res.error(httpStatus.BAD_REQUEST, false, "FCM token is required");
    }
...
userController.sendNewsApprovedNotification = async (newsId, newsTitle) => {
  try {
    ...
    console.log(
      `Notification sent to ${audienceIds.length} audience members for news: ${newsTitle}`,
    );
    return true;
  } catch (error) {
...
userController.deleteAccount = async (req, res) => {
  try {
    ...
    if (!isValidPassword) {
      return res.error(httpStatus.UNAUTHORIZED, false, "Invalid password");
    }

    // Delete the user
    await user.destroy();

    return res.success(httpStatus.OK, true, "Account deleted successfully");
  } catch (error) {
