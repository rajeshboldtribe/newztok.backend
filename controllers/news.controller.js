    ...
    try {
      if (status === "approved") {
        ...
        try {
          ...
          if (news.state) {
            const stateTopic = `state_${news.state
              .toLowerCase()
              .replace(/\s+/g, "_")}`;
            await notificationService.sendToTopic(
              stateTopic,
          ...
          if (news.district) {
            const districtTopic = `district_${news.district
              .toLowerCase()
              .replace(/\s+/g, "_")}`;
            await notificationService.sendToTopic(
              districtTopic,
          ...
          if (news.category) {
            const categoryTopic = `category_${news.category
              .toLowerCase()
              .replace(/\s+/g, "_")}`;
            await notificationService.sendToTopic(
              categoryTopic,
      ...
      try {
        ...
        if (contentType === "standard" || news.contentType === "standard") {
          if (
            ...
            if (
              news.featuredImage &&
              news.featuredImage !== "/uploads/images/default.jpg"
            ) {
...
newsController.deleteNews = async (req, res) => {
  try {
    ...
    if (
      userRole !== "admin" &&
      (userRole !== "editor" || news.editorId !== userId)
    ) {
      return res.error(
        httpStatus.FORBIDDEN,
        false,
  ...
  try {
    ...
    uploadFields(req, res, async function (err) {
      ...
      try {
        ...
        console.log("Admin creating news - Request body:", {
          title,
          content,
          category,
          contentType,
...
newsController.adminEditPendingNews = async (req, res) => {
  try {
    ...
    if (!news) {
      return res.error(httpStatus.NOT_FOUND, false, "News post not found");
    }
