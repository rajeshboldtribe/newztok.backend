...
newsController.updateNews = async (req, res) => {
    try {
        ...
        const news = await News.findOne({
            where: { 
                id: newsId,
                journalistId: journalistId // Ensure the journalist owns this article
            }
        });
        ...
        // Check if the news is already approved
        ...
        uploadFields(req, res, async function(err) {
            ...
            try {
                ...
                try {
                    await notificationService.sendToRole('editor', 
                        'Article Updated', 
                        `${journalist.username} has updated an article: "${news.title}"`,
                        {
                ...
                return res.success(httpStatus.OK, true, "News updated successfully", news);
        ...
        console.error('Update news outer error:', error);
        return res.error(
            httpStatus.INTERNAL_SERVER_ERROR,
            false,
            "Error processing request: " + error.message
