...
dashboardController.getAllUsers = async (req, res) => {
  try {
    ...
    return res.success(httpStatus.OK, true, "Users fetched successfully", {
...
dashboardController.deleteUser = async (req, res) => {
  try {
    ...
    return res.success(httpStatus.OK, true, "User deleted successfully", {
      id: userId,
      role: user.role,
    });
  } catch (error) {
    ...
    upload.single("profilePicture")(req, res, async function (err) {
      ...
      try {
        ...
        try {
          if (
            notificationService &&
            typeof notificationService.sendToUsers === "function"
          ) {
  ...
  try {
    ...
    const formattedNews = news.map((item) => ({
      ...
      author: {
        ...
        location:
          `${item.journalist?.assignedState || ""}, ${item.journalist?.assignedDistrict || ""}`.trim(),
      },
