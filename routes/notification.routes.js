...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await notificationService.sendToTopic(topic, title, body, {
  ...
  async (req, res) => {
    try {
      ...
      if (allUsers.length === 0) {
        return res.success(200, true, "No users with FCM tokens found", {
          usersCount: 0,
        });
      }
      ...
      return res.success(
        200,
        true,
        `Test notification sent to ${userIds.length} users`,
        {
